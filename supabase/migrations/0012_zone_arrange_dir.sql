-- Phase 11 — Settings: per-zone merge build direction. Explicit host choice that
-- overrides the smoking-based heuristic when a party is seated and tables snap
-- together. Nullable ('horizontal' | 'vertical' | null = auto).
--
-- replace_layout has a fixed zone column list, so recreate it to carry the new
-- field (it already carries `bookable` from 0009).

alter table zones add column if not exists arrange_dir text;
...
begin
  ...
  insert into zones (id, restaurant_id, name, color, position, size, parent_id, smoking, allow_table_relocation, bookable, arrange_dir)
  select x.id, p_restaurant_id, x.name, x.color, x.position, x.size, x.parent_id, x.smoking, x.allow_table_relocation, x.bookable, x.arrange_dir
  from jsonb_to_recordset(p_zones) as x(
    id text, name text, color text, position jsonb, size jsonb,
    parent_id text, smoking text, allow_table_relocation boolean, bookable boolean, arrange_dir text
  );
