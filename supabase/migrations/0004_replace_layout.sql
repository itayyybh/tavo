-- Phase 9 — Transactional layout save. The editor autosaves the whole document;
-- doing delete+insert as one plpgsql function keeps it in a single transaction,
-- so a failed save can never leave a restaurant with a half-wiped floor.
--
-- Rows arrive as jsonb arrays of ROW-shaped objects (snake_case keys matching
-- the columns). `restaurant_id` is taken from the argument, never trusted from
-- the payload. Membership is enforced before any write.
...
begin
  ...
  insert into zones (id, restaurant_id, name, color, position, size, parent_id, smoking, allow_table_relocation)
  select x.id, p_restaurant_id, x.name, x.color, x.position, x.size, x.parent_id, x.smoking, x.allow_table_relocation
  from jsonb_to_recordset(p_zones) as x(
    id text, name text, color text, position jsonb, size jsonb,
    parent_id text, smoking text, allow_table_relocation boolean
  );
