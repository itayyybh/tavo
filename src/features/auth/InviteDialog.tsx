 ...
 * this restaurant (see `redeem_invite`). Controlled by its host (the account
...
export function InviteDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // Reset on close so the next open never shows a stale link.
  const handleClose = () => {
    setCode(null)
    setError(null)
    setCopied(false)
  ...
  return (
    <Dialog open={open} onClose={handleClose} title="Invite a manager">
